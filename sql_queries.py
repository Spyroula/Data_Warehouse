...
staging_events_copy = ("""
    COPY staging_events FROM {data_s3_bucket}
    CREDENTIALS 'aws_iam_role={role_arn}'
    REGION 'us-west-2'
    FORMAT AS JSON {json_path};
    """).format(data_s3_bucket=config['S3']['LOG_DATA'], role_arn=config['IAM_ROLE']['ARN'], json_path=config['S3']['LOG_JSONPATH'])


staging_songs_copy = ("""
    COPY staging_songs FROM {data_s3_bucket}
    CREDENTIALS 'aws_iam_role={role_arn}'
    REGION 'us-west-2'
    FORMAT AS JSON 'auto';
""").format(data_s3_bucket=config['S3']['SONG_DATA'], role_arn=config['IAM_ROLE']['ARN'])
